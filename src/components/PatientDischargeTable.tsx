    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
      ...
      <div className="px-5 py-3 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-2">
          <span className="font-bold text-slate-700">Tampilan Peran:</span>
          {activeRole === 'ruangan' && (
            <span className="px-2.5 py-1 rounded-lg bg-teal-100 text-teal-800 font-bold flex items-center gap-1.5">
              <Building2 className="w-3.5 h-3.5" /> Ruang Rawat Inap (Pemantauan Status Validasi)
            </span>
          )}
          {activeRole === 'tpp' && (
            <span className="px-2.5 py-1 rounded-lg bg-sky-100 text-sky-800 font-bold flex items-center gap-1.5">
              <CreditCard className="w-3.5 h-3.5" /> TPP & Informasi (Input Pembiayaan, Hak, Naik & Titip Kelas)
            </span>
          )}
          {activeRole === 'billing' && (
            <span className="px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-800 font-bold flex items-center gap-1.5">
              <Receipt className="w-3.5 h-3.5" /> Kasir Billing (Finalisasi Pemulangan Pasien)
            </span>
          )}
          {activeRole === 'monitor' && (
            <span className="px-2.5 py-1 rounded-lg bg-slate-800 text-white font-bold">
              Pantauan Terpadu Seluruh Alur (Ruangan ➔ TPP ➔ Billing)
            </span>
          )}
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 text-xs text-amber-900 bg-amber-100/90 border border-amber-300 px-2.5 py-1 rounded-lg font-medium">
            <span className="w-2 h-2 rounded-full bg-amber-500 animate-ping"></span>
            <span className="font-bold">Prioritas:</span>
            <span>Pasien belum validasi TPP diletakkan di posisi paling atas dengan sorotan warna kuning emas</span>
          </div>
        ...
        <table className="w-full text-left text-xs">
          <thead className="bg-slate-100/70 border-b border-slate-200 text-slate-600 font-bold uppercase text-[10px] tracking-wider">
            <tr>
              ...
              <th className="py-3 px-4">Status Alur 3 Level</th>
                ...
                >
                  ...
                  <td className="py-3.5 px-4 min-w-[200px]">
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-center gap-1 text-[10px]">
                        ...
                        }`} title="Finalisasi Kasir Billing">
                  ...
                  <td className="py-3.5 px-4 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      ...
                      {(activeRole === 'tpp' || activeRole === 'monitor') && (
                        patient.statusAlur === 'menunggu_tpp' ? (
                          <button
                      ...
                      {(activeRole === 'billing' || activeRole === 'monitor') && (
                        patient.statusAlur === 'menunggu_billing' ? (
                          <button
